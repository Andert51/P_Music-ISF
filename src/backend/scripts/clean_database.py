    ...
    if uploads_dir.exists():
        for item in uploads_dir.iterdir():
            ...
            try:
                if item.is_dir():
                    shutil.rmtree(item)
                    print(f"Eliminado directorio: {item.name}")
                else:
                    item.unlink()
...
# Función para limpiar la base de datos, respetando foreign keys 
...
def main():
    ...
    try:
        ...
        print("    Todos los archivos uploads eliminados")
